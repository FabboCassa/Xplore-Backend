...
- **Enrichment:** Calls the **Wikipedia API** to fetch and append summaries, descriptions, and images to the POIs.
- Returns the compiled list to the client.
- **Zero POI storage:** The backend DB only stores Users, Authentication, User Progress/Achievements, Saved Routes, and core app metadata. It does not store the worldwide map of museums.
...
## 3. Saved Routes & Navigation
- While the global POI map catalog remains fully dynamic and stateless, **User Routes** (including generated navigation routes, waypoints, and completed status) are durably stored in the backend SQL database.
- Routes can be grouped, saved in user profiles, and shared broadly via sharing links.
